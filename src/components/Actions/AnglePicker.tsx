...
const mergeStyles = (style1: any, style2: any) => {
    return {
        ...style1,
        ...style2,
    };
...
const AngleRange = ({
    ...
    const getHandlerXY = useCallback(
        (angle: number, handlerRadiusOffset: number) => {
            const alpha = angle % 360;
            const x =
                (radius + handlerRadiusOffset) *
    ...
    const onMouseMove = useCallback(() => {
        ...
        if (isCenterHandlerActive) {
            ...
            const offsetAngle = Math.ceil(
                updatedCenterAngle - centerHandler.angle
            );
            const updatedFromAngle = modulus360(
                fromAngleHandler.angle + offsetAngle
            );
            const updatedToAngle = modulus360(
                toAngleHandler.angle + offsetAngle
            );
            ...
            if (
                ...
                (limitTo &&
                    (limitTo.min > updatedToAngle ||
                        limitTo.max < updatedToAngle))
            ) {
                return;
            }
            setFromAngleHandler({
        ...
        onChange({
            from: fromAngleHandler.angle,
            to: toAngleHandler.angle,
        });
    }, [
        isDisabled,
                ...
                style={mergeStyles(
                    {
                        ...
                        borderBottomRightRadius: isQuarterCircle
                            ? 0
                            : 2 * radius,
                        borderBottomLeftRadius: isQuarterCircle
                            ? 0
                            : 2 * radius,
                    ...
                    style={mergeStyles(
                        {
                            ...
                            transformOrigin: isQuarterCircle
                                ? "100% 100%"
                                : "50% 100%",
                        },
                        styles.offsetsSegment
                    )}
                >
                    <div
                    ...
                    style={mergeStyles(
                        {
                            ...
                            transform: `translate(${
                                centerHandler.x - handlerRadius
                            }px, ${centerHandler.y - handlerRadius}px)`,
                            opacity: isCenterHandlerActive ? 0.5 : 1,
                            cursor: isCenterHandlerActive ? "move" : "pointer",
                        },
                        styles.centerAngleHandler
                    )}
                    ...
                    style={mergeStyles(
                        {
                            ...
                            transform: `translate(${
                                ...
                                fromAngleHandler.y - offsetHandlerRadius + 1
                            }px) rotate(${fromAngleHandler.angle}deg)`,
                            borderTopLeftRadius: 2 * offsetHandlerRadius,
                            opacity: isFromAngleHandlerActive ? 0.8 : 1,
                            cursor: isFromAngleHandlerActive
                                ? "move"
                                : "pointer",
                        },
                        styles.offsetAngleHandler
                    )}
...
export type AngleClasses = {
    root?: string;
    fullRange?: string;
    offsetsSegment?: string;
    offsetsSegmentRemove?: string;
    relativeAxis?: string;
    offsetAngleHandler?: string;
    centerAngleHandler?: string;
};

export type AngleRangeProps = {
    classes?: AngleClasses;
    value?: Range;
    onChange?: (range: Range) => void;
    isDisabled?: boolean;
    limitFrom?: Limit | null;
    limitTo?: Limit | null;
    radius?: number;
